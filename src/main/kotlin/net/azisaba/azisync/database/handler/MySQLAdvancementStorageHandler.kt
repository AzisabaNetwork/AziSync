...
class MySQLAdvancementStorageHandler(private val plugin: AziSync) : AdvancementStorageHandler {

    private val tableName: String
        get() = plugin.config.getString("database.TablesNames.advancementTableName", "azisync_advancement")!!
        ...
        return try {
            plugin.databaseManager.getConnection().use { conn ->
                val sql = """
                    ...
                    (`player_uuid`, `player_name`, `advancements`, `last_seen`, `sync_complete`)
                    VALUES(?, ?, ?, ?, ?)
                """.trimIndent()
                conn.prepareStatement(sql).use { stmt ->
                    stmt.setString(1, uuid.toString())
            ...
            plugin.logger.warning("Error creating advancement account for ${playerName}: ${e.message}")
            false
        }
    }
    ...
    override fun setSyncStatus(uuid: UUID, playerName: String, status: String): Boolean {
        return try {
            ...
            plugin.logger.warning("Error setting advancement sync status for ${playerName}: ${e.message}")
            false
        }
    }
    ...
    override fun setData(uuid: UUID, playerName: String, advancements: String, syncStatus: String): Boolean {
        ...
        return try {
            plugin.databaseManager.getConnection().use { conn ->
                val sql = """
                    UPDATE `$tableName`
                    SET `player_name` = ?, `advancements` = ?, `sync_complete` = ?, `last_seen` = ?
                    WHERE `player_uuid` = ?
                """.trimIndent()
                conn.prepareStatement(sql).use { stmt ->
                    stmt.setString(1, playerName)
                    stmt.setString(2, advancements)
            ...
            plugin.logger.warning("Error saving advancement data for ${playerName}: ${e.message}")
            false
        }
    }
}
